...
//--------------------------------------------------------------
	...
	{
		maxGreyLevel= maxGrey;
		this.width= newImage.length;
		this.height= newImage[0].length;
		img= new int[width][];
		for(int i= 0; i<width; i++)
			img[i]= newImage[i].clone();
	}
...
//---------------------------------------------------------------
	...
	//Rotation operation on an image
	public static BWImage rotate(BWImage theImg, int degrees)
	{
		int[][] newImg= new int[theImg.height][theImg.width];
		...
		for(int i= 0; i<theImg.width; i++)
		{
			for(int j= 0; j<theImg.height; j++)
			{
				...
				newImg[theImg.height-1-j][i]= theImg.img[i][j];
			}
		}
		
		BWImage newImage=  new BWImage(newImg, theImg.maxGreyLevel);
		...
		{
			return rotate(newImage, degrees-90);
		}
	}
	
	public BWImage FlipHorizontal()
	{
		...
		for(int i= 0; i<width; i++)
		{
			for(int j= 0; j<height; j++)
			{
				newImg[i][j]= img[i][height - 1 -j];
			}
		}
		
		BWImage newImage= new BWImage(newImg, maxGreyLevel);
		return newImage;
	}
	
	public BWImage FlipVertical()
	{
		int[][] newImg= new int[width][height];
	...
	public BWImage MagnifyR(float size)
	{	
		int[][] newImage= new int[(int)(width * size)][(int)(height*size)];
		...
		for(int i= 0; i< width; i++)
		{
			for(int j=0; j<height; j++)
			{
				for(int x= 0; x<size; x++)
				{
					...
					{
						if(((i+x) < (width*size)) && ((j+y) < (height*size)))
						{
							newImage[(int)(i*size) + x][(int)(j*size) + y]= img[(int)i][(int)j];
						}
					}
				}
			}
		...
		BWImage newImg= new BWImage(newImage, maxGreyLevel);	
		return newImg;
	}
	
	
	//magnification by interpolation
	public BWImage MagnifyI(float size)
	{
		int[][] newImage= new int[(int)(width * size)][(int)(height*size)];
		...
		for(int i= 0; i< width; i++)
		{
			for(int j=0; j<height; j++)
			{
				newImage[(int)(i*size)][(int)(j*size)]= img[i][j];
				if(((i*size + 1) < (width*size)) && ((j*size + 1)< (height*size)))
				{
					int x= i+1;
					int y= j+1;
					if(x > width-1) 
					...
					if(y > height-1)
						y= height -1;
					newImage[(int)(i*size)+1][(int)(j*size)]= findMedium(img[i][j], img[x][j]);
					newImage[(int)(i*size)][(int)(j*size)+1]= findMedium(img[i][j], img[i][y]);
					if(i*size -2 > 0)
					{
						newImage[(int)(i*size)-1][(int)(j*size)+1]= findMedium(newImage[(int)(i*size)-2][(int)(j*size+1)],newImage[(int)(i*size)][(int)(j*size)+1]);
					}
				}
			}
		}
	...
	public static int findMedium(int num1, int num2)
	{
		int theNum;
		
		theNum= (int)((num1 + num2) /2); 
		
		return theNum;
	}
	
	//simple reduction
	public BWImage Reduction(float decrease)
	{
		int[][] newImage= new int[(int)(width/decrease)][(int)(height/decrease)];
		
		for(int i= 0; i < newImage.length; i++)
		{
			for(int j= 0; j<newImage[0].length; j++)
			{
				newImage[i][j]= img[(int)(i*decrease)][(int)(j*decrease)];
			}
		}
	...
	public BWImage ReductionA(float decrease)
	{
		int[][] newImage= new int[(int)(width/decrease)][(int)(height/decrease)];
		
		int[] theSquare= new int[(int)(decrease*decrease)];
		for(int i= 0; i<newImage.length; i++)
		{
			for(int j= 0; j<newImage[0].length; j++)
			{
				for(int x= 0; x< decrease; x++)
				{
					for(int y= 0; y< decrease; y++)
					{
						theSquare[(int)((x*decrease) + y)]= img[(int)((i*decrease)+x)][(int)((j*decrease)+y)];
					}
				}
				newImage[i][j]= findAverage(theSquare);
			}
		}
	...
	//reduction with medians
	public BWImage ReductionM(float decrease)
	{
		int[][] newImage= new int[(int)(width/decrease)][(int)(height/decrease)];
		...
		for(int i= 0; i<newImage.length; i++)
		{
			for(int j= 0; j<newImage[0].length; j++)
			{
				...
				bubbleSort(theSquare);
				
				newImage[i][j]= theSquare[(int)(theSquare.length/2)]; 
			}
		}
